...
Copilot::Copilot()
{
    // Init params
    mParams.Prompt = "";
    mParams.PromptSent = false;
    mParams.hairReceived = false;
}



void Copilot::Generate(std::string prompt)
{
    // Generation code

    //std::string input = "Short Buzzed Cut with Curly Top.";
    std::string result = run_python_code(prompt);
...
std::string Copilot::run_python_code(const std::string &input_str) {
